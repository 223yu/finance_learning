class User < ApplicationRecord
  ...
  # 年度データが存在すればtrueを返す
  def has_year?
    year != 0 # 初回データ作成後は必ず年度データがyearに格納されている前提（前提条件が変わった場合DB検索を検討）
  end
  ...
  def start_date(start_month)
    Time.new(year, start_month, 1).beginning_of_month
  end

  # ユーザの持つ年度、入力月から月末日を返す
  def end_date(end_month)
    Time.new(year, end_month, 1).end_of_month
  end
  ...
  def update_year(year)
    next_year = year + 1
    now_accounts = Account.where(user_id: id, year: year)
    now_accounts.each do |account|
      ...
      # 翌年度に同じコードの科目が存在する場合
      if Account.exists?(user_id: id, year: next_year, code: account.code)
        next_account = Account.find_by(user_id: id, year: next_year, code: account.code)
        ...
        next_account = Account.create(user_id: id, year: next_year, code: account.code, name: account.name, total_account: account.total_account, opening_balance_1: ending_balance)
  ...
  def has_journal_in_this_year?(account)
    range = start_date_to_end_date(1, 12)
    Journal.where(user_id: id, date: range, debit_id: account.id).or(Journal.where(user_id: id, date: range, credit_id: account.id)).exists?
  end

  # 合計科目から勘定科目の一覧を返す
  def accounts_index_from_total_account(total_account)
    accounts_array = []
    accounts = Account.where(user_id: id, year: year, total_account: total_account)
    accounts.each do |account|
      account_array = ["#{account.code} #{account.name}", account.code]
      accounts_array.push(account_array)
    end
  ...
  def journal_index_from_self_code(self_code, date, offset)
    self_id = code_id(self_code)
    Journal.where(user_id: id, date: date, debit_id: self_id).or(Journal.where(user_id: id, date: date, credit_id: self_id)).order(date: 'DESC').limit(15).offset(offset)
  end

  # 総勘定元帳にて科目に対する仕訳の一覧を返す
  def journal_index_from_self_code_in_ledger(self_code, date)
    self_id = code_id(self_code)
    Journal.where(user_id: id, date: date, debit_id: self_id).or(Journal.where(user_id: id, date: date, credit_id: self_id)).order(:date)
  end
  ...
  # 学習済みであればtrueを返す
  def learned?(content)
    Learning.find_by(user_id: id, content_id: content.id).present?
  end
end
